...
fn main() {
    ...
    let grammar_fuel_pest_path = manifest_dir.join("src/grammar.pest");
    let grammar_rs_path = manifest_dir.join("src/grammar.rs");
    let out_dir = PathBuf::from(env::var("OUT_DIR").unwrap());
    let hash_path = out_dir.join("fuel_pest_hash.sha1");

    // If `grammar.pest` exists (we're building from git sources)
    if grammar_fuel_pest_path.exists() {
        ...
        let current_grammar = fs::read_to_string(grammar_fuel_pest_path).unwrap();
            ...
            || old_hash.as_ref().map(|it| it.trim()) != Some(current_hash.trim())
        {
            ...
            let status = Command::new(manifest_dir.join("../target/debug/fuel_pest_bootstrap"))
                .spawn()
                .unwrap_or_else(|_| {
                    panic!(
                        "Bootstrap failed because no bootstrap executable was found. \
                         Please run `cargo build --package fuel_pest_bootstrap` or `cargo bootstrap` \
                         and then try again.",
                    )
                })
                .wait()
